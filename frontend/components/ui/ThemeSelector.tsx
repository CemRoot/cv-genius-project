...
const ThemeSelector: React.FC<ThemeSelectorProps> = ({
  ...
  const handlePreviewClick = (themeId: CoverLetterTheme) => {
    ...
    // Scroll to make modal visible and center it on screen
    setTimeout(() => {
      // Find the cover letter preview content and scroll to it
      ...
      if (previewContent) {
        ...
        // Also ensure modal content starts from top
  ...
  const closePreview = () => {
    ...
    // Scroll back to the theme selector section after modal closes
    setTimeout(() => {
            ...
            <div className="flex-1 overflow-y-auto p-2" id="cover-letter-preview-content">
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm mx-auto text-sm leading-relaxed max-w-3xl min-h-full">
                ...
                <div className={clsx(
                  ...
                  previewTheme.id === 'modern' && 'border-b-2 border-blue-500 pb-3',
                  previewTheme.id === 'academic' && 'text-right border-b border-black pb-2'
                )}>
                  <div className={clsx(
                    'font-bold mb-2',
                    previewTheme.id === 'classic' && 'text-base',
                    previewTheme.id === 'modern' && 'text-lg text-blue-600',
                    previewTheme.id === 'academic' && 'text-base italic'
                  )}>
                    {previewTheme.mockup.header}
                  </div>
                  <div className="text-sm text-gray-600 whitespace-pre-line leading-tight">
                ...
                {previewTheme.mockup.recipient && (
                  <div className="mb-4 text-sm whitespace-pre-line leading-tight">
                    {previewTheme.mockup.recipient}
                  </div>
                )}
                ...
                {previewTheme.mockup.subject && (
                  <div className="mb-4 font-bold text-blue-600 underline text-sm">
                    {previewTheme.mockup.subject}
                  </div>
                )}
            ...
            <div className="bg-gray-50 p-4 border-t flex-shrink-0">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div className="text-xs text-gray-600">
                  Professional cover letter format • Select your preferred style
                </div>
                <button
                  type="button"
                  onClick={(e) => {
                  ...
                  className={clsx(
                    'px-6 py-2 rounded-lg font-medium text-white transition-all w-full sm:w-auto',
